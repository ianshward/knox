...
function setAuth(date, method, filename, headers) {
...
function checkCredentials(fn) {
  ...
  var req = function(url, cb) {
    ...
    http.get(url, function(res) {
      if (typeof res === 'undefined')
        return fn(new Error('metadata API response undefined'));
  ...
  if (this.key == null || this.secret == null || lapse > 0) {
    req(url, function(roles) {
      req(url + roles.split('\n')[0], function(json) {
        try {
        ...
        self.secret = json.SecretAccessKey;
        self.key = json.AccessKeyId;
        self.token = json.Token;
        self.expires = json.Expiration;
        fn(null, self);
      });
    });
  } else {
...
Client.prototype.request = function(method, filename, headers, fn){
  var self = this;
  ...
  if ('function' == typeof fn) {
    checkCredentials.call(this, function(err, res) {
      ...
      if (self.token != null) headers['x-amz-security-token'] = self.token;
      headers.Authorization = setAuth.call(self, date, method, filename, headers);
    ...
    headers.Authorization = setAuth.call(this, date, method, filename, headers);
    options.headers = headers;
...
Client.prototype.get = function(filename, headers, fn){
  return this.request('GET', filename, headers, fn);
};
...
Client.prototype.getFile = function(filename, headers, fn){
  ...
  this.get(filename, headers, function(err, req) {
    if (err) return fn(err);
    registerReqListeners(req, fn);
    req.end();
  });
